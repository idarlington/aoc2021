...
pub fn main() {
    ...
    let crab_positions = content_value
        .iter()
        .fold(HashMap::new(), |mut positions, pos| {
    ...
    let all_positions: HashMap<i32, Position> = (min_key.clone()..=max_key.clone()).fold(
        crab_positions.clone(),
        |mut positions_with_gaps_filled, pos| {
...
fn day_7_part_1(all_positions: HashMap<i32, Position>, crab_positions: HashMap<i32, Position>) {
    ...
    let least_fuel_position: Option<(i32, Position)> =
        calculate_least_fuel_position(updated_positions);
...
fn populate_positions_fuel(
    all_positions: HashMap<i32, Position>,
    crab_positions: HashMap<i32, Position>,
    binomial: bool,
) -> HashMap<i32, Position> {
    all_positions
        .iter()
        .fold(all_positions.clone(), |mut positions, (current_key, _)| {
            ...
            for (key, position) in crab_positions.clone() {
                let fuel = {
                    if binomial {
                        (binomial_coefficient((current_key.clone() - key.clone()).abs()) as i32)
                            * position.multiplier
                    } else {
...
fn calculate_least_fuel_position(positions: HashMap<i32, Position>) -> Option<(i32, Position)> {
    positions.iter().fold(
        None,
        |least_fuel_position, (current_key, current_position)| match least_fuel_position {
            None => Some((*current_key, *current_position)),
            Some((key, position)) => {
                if (current_position.fuel) < (position.fuel) {
                    Some((*current_key, *current_position))
                } else {
                    Some((key, position))
                }
            }
